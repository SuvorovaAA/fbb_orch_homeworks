...
Чтобы сопоставить названия генов идентификаторам UbiProt, воспользовалась 
не ID Mapping, так как тот не позволяет ограничить поиск одним видом, 
а `https://rest.uniprot.org/uniprotkb/search`. Иначе выдача оказывается
чересчур большой, что вообще не очень осмысленно.

После "постучалась" в Pfam, чтобы получить Pfam ID для каждого из UniProt ID:
`https://www.ebi.ac.uk/interpro/api/protein/uniprot/{uniprot_id}/entry/pfam`.
Потом уже для них искала идентификаторы доменов: `https://www.ebi.ac.uk/interpro/api/entry/pfam/{pfam_id}?ida`.

После каждого запроса "ждала" по 0.5-1 секунде, чтобы не закидывать базы
данных запросами совсем уж без остановок.

Не могу утверждать, что не напутала ничего в скрипте, но он, кажется, работает... 
Хотя в процессе столкнулась с тем, что к некоторым сервисам нельзя обратиться
нормально без VPN, так что его оказалось проще запускать блоками.